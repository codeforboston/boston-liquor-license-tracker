...
def parse_entity(entity: str) -> dict[str, str | None]:
    ...
    for line in lines:
        ...
        if "doing business as:" in lower_line:
            dba_match: re.Match[str] | None = re.search(r"doing business as:\s*(.+)", line, re.IGNORECASE)
            if dba_match:
                result["dba_name"] = dba_match.group(1).strip()

        if "license" in lower_line:
            license_match: re.Match[str] | None = re.search(r"license\s*#:\s*([\w\-]+)", line, re.IGNORECASE)
            if license_match:
                result["license_number"] = license_match.group(1).strip()

        if not result["address"] and re.search(r"\d{5}$", line):
            result["address"] = line.strip()
            zip_match: re.Match[str] | None = re.search(r"\b(\d{5})\b", line)
            if zip_match:
                ...
                if block["type"] == 0:
                    for line in block["lines"]:
                        ...
                        for span in line["spans"]:
                            ...
                            heading_match: re.Match[str] | None = re.match(heading_regex, span_text)
